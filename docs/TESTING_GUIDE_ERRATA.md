...
### 问题 6：`start_watching` 工具参数错误（设计 vs 实现不匹配）
...
**根本原因**：
- 设计文档中规划了 `watch_patterns` 和 `ignore_patterns` 参数
- 实际实现中没有加入这些参数
- 测试文档照搬了设计文档，没有验证实际实现

**修正后的说明**：
- ✅ `start_watching` 只需要 `project_path` 参数
- ✅ 会监控整个项目目录下的所有文件变化
- ✅ 文件过滤功能（watch_patterns、ignore_patterns）计划在 v0.3+ 中实现
- ✅ 当前版本使用默认忽略列表：node_modules, .git, dist, build, coverage, .agentmx.db

**教训**：
设计文档中的功能不一定都在实现中。编写测试文档时必须：
1. 查看实际的 MCP Server 实现代码
2. 验证工具的实际参数和返回格式
3. 不要假设设计文档中的所有功能都已实现
...
| 问题 | 发现日期 | 修复日期 | 版本 |
|------|---------|---------|------|
| 问题 1-5 | 2026-05-19 | 2026-05-19 | v0.2.0 |
| 问题 6 | 2026-05-19 | 2026-05-19 | v0.2.0 |
